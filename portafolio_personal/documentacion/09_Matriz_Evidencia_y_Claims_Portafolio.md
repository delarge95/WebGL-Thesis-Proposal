...
| Onboarding UX | La app incluye un onboarding procedural ligero que demuestra gestos y microinteracciones reales de desktop y mobile | `OnboardingController.cs`, `OnboardingAnimationView.cs`, `PLAN_ANIMACIONES_ONBOARDING.md` | Publicable con guardrails | clip de `Navigate`, clip de menus/submenus, comparativa PC/mobile | que usa video prerenderizado o que reemplaza testing/telemetria real de onboarding |
...
| Fastener subsystem | El visor resuelve fasteners por metadata tecnica e instancia detalle solo bajo demanda | `FastenerRegistry.cs`, `FastenerInspectionManager.cs`, `HolybroFastenerCatalogBuilder.cs`, catalogos `holybro_fastener_*` | Publicable con guardrails | clip proxy -> detail, ficha tecnica de fastener, captura de catalogos/tooling | que las mallas Blender finales ya reemplazaron todos los placeholders o que existe paridad CAD cerrada |
...
| El onboarding final no depende de media pesada y aun asi explica interacciones complejas | `OnboardingAnimationView.cs`, `PLAN_ANIMACIONES_ONBOARDING.md` | Util para posiciones con foco en UX tecnica, UI systems y onboarding de producto |
