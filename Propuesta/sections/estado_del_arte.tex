...
\subparagraph{Desventajas} Requiere implementación manual de \textit{pipeline} de assets (cargadores GLTF/OBJ personalizados), gestión de memoria explícita, ausencia de editor visual para artistas no-programadores.
...
\subparagraph{Ventajas} Soporte nativo de TypeScript, editor visual integrado (Babylon.js Playground), sistema de física robusto (Cannon.js/Ammo.js), migración temprana a WebGPU implementada (soporte desde v5.0). Fransson et al. (2024) en la IEEE Conference on Games demostraron que WebGPU como backend en motores de juego muestra tiempos de cuadro consistentemente mejores que WebGL, validando la estrategia de migración temprana de Babylon.js.
...
\subparagraph{Ventajas} Editor en navegador con colaboración en tiempo real (similar a Google Docs pero para desarrollo de juegos), asset \textit{pipeline} completo con optimización automática (compresión de texturas, bundling), deployment con un click a CDN global, tamaño de runtime pequeño (~500KB gzip).
...
\subparagraph{Three.js/Babylon.js/PlayCanvas} Tiempo de desarrollo excesivo para implementar \textit{pipeline} de optimización completo (Occlusion Culling automático, sistema LOD de 3 niveles, Asset Bundles con carga asíncrona) desde cero, estimado en $>$200 horas adicionales de ingeniería.
