...
\paragraph{OE4: Documentación y Despliegue.} Se generó documentación técnica exhaustiva ($\sim$1\,060+$ líneas de documentación de arquitectura, un inventario completo de módulos, y documentación del \textit{pipeline} de \textit{shaders}). El código se gestionó en un repositorio Git con~316~\textit{commits} estructurados. El despliegue se validó para navegadores Chrome, Firefox, Edge y Safari en dispositivos de escritorio y móviles.
...
\begin{enumerate}
    ...
    \item \textbf{Herramientas de Ensamblaje.} Los módulos \texttt{AssemblyGuideManager}, \texttt{AssemblyChecklist}, \texttt{BillOfMaterialsManager}, \texttt{ConnectionPointsViewer} y \texttt{AnnotationSystem} fueron desarrollados como código funcional con sus respectivas interfaces (\texttt{AssemblyStepUI}) y datos (\texttt{AssemblyGuideData}). Sin embargo, no fueron integrados en la interfaz de usuario de la versión actual del prototipo. Su activación requiere: (a)~configuración de componentes \texttt{ConnectionPointMarker} en los objetos de la escena, (b)~persistencia de anotaciones, y (c)~integración de los paneles UI con el flujo de navegación principal.
    ...
    \item \textbf{Captura de Pantalla.} Integrar un módulo de captura que genere imágenes PNG vía base64 en WebGL y descarga directa al dispositivo del usuario.

    \item \textbf{Asset Bundles.} Si bien la arquitectura del sistema se diseñó para ser extensible a múltiples modelos de drones mediante \texttt{ScriptableObjects}, la carga dinámica de modelos mediante \textit{Asset Bundles} no fue implementada en la versión actual. Esta funcionalidad, combinada con un CMS, permitiría agregar nuevos modelos sin recompilar la aplicación.

    \item \textbf{Pipeline Procedural (Houdini).} SideFX Houdini fue evaluado como herramienta de procesamiento procedural para la conversión automatizada CAD$\rightarrow$mesh y generación de LODs. Para el modelo único del X500~V2, Blender ofreció resultados superiores. Sin embargo, se recomienda Houdini para futuros pipelines con múltiples modelos de hardware.

    \item \textbf{Subsistema Térmico.} El subsistema térmico híbrido, implementado como contribución adicional al proyecto, constituye una base de simulación cualitativa con verificación Wolfram. Para futuras iteraciones, se recomienda la calibración con datos experimentales reales y la integración de un \textit{slider} de carga del usuario en la interfaz.
\end{enumerate}
