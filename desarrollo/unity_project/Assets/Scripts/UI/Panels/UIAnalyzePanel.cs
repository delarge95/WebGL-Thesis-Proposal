...
namespace WebGL.UI.Panels
{
    public class UIAnalyzePanel
    {
        ...
        private ViewMode _lastViewMode = ViewMode.Realistic;
        ...
        {
            UpdateShaderButtonVisual(_lastViewMode);
        }
        ...
        private void UpdateShaderButtonVisual(ViewMode mode)
        {
            ...
            _shaderBtn.tooltip = isActive
                ? $"{AppLanguageManager.TranslateStatic("View")}: {FormatViewModeLabel(mode)}"
                : AppLanguageManager.TranslateStatic("Toggle Render Mode");
        }

        private static string FormatViewModeLabel(ViewMode mode)
        {
            string key = mode switch
            {
                ViewMode.XRay => "X-RAY",
                ViewMode.Blueprint => "BLUEPRINT",
                ViewMode.SolidColor => "SOLID",
                ViewMode.Wireframe => "WIRE",
                ViewMode.Ghosted => "GHOST",
                ViewMode.Thermal => "THERMAL",
                _ => "REALISTIC"
            };

            return AppLanguageManager.TranslateStatic(key);
        }
