# Manual de Usuario — TwinSight X500
...
Cerrar ficha o subpanel | tecla `Esc` o botón de cierre | deslizar hacia abajo sobre la ficha o usar el botón de cierre 

Restablecer vista | botón `RESET VIEW` | botón `RESET VIEW` 

Regresar al Hero | botón `HOME` o botón Atrás por capas | botón `HOME` o botón Atrás por capas 
...
El botón Atrás del navegador se maneja por capas: primero cierra tutorial, ficha o submenús; luego vuelve del canvas de exploración al Hero; solo desde el menú principal muestra la confirmación de salida de la app.
...
Cuando la ficha está abierta, su superficie bloquea la interacción con el dron para evitar orbit, selección o zoom accidentales. Para cerrarla en móvil, se puede deslizar hacia abajo desde la zona superior o desde el área visible del panel.
...
Cuando la selección corresponde a un fastener individual, la ficha añade datos técnicos específicos como familia, métrica, longitud, material, referencia CAD y pieza canónica más cercana dentro del dron. La escena combina mallas conservadas, proxies ligeros y reconstrucción modular aplicada a tornillos para mantener legibilidad y rendimiento en WebGL.
...
Además del clic simple, la build final conserva dos interacciones útiles:
    ...
    * **Doble clic o doble toque sobre una pieza, subpieza, fastener u hotspot:** aísla o desaísla el nivel actual sin abrir automáticamente la ficha.
...
La visualización térmica se alimenta del estado operativo del dron. Por eso:
    ...
    * las hélices permanecen entre las piezas más frías: solo pueden mostrar una leve activación en el centro por cercanía con el motor, mientras los extremos deben conservar una lectura fría;
...
## Limitaciones visibles del cierre
    ...
    * la sustitucion visual por un modelo Blender futuro no debe cambiar seleccion, aislamiento, filtros, hotspots ni comportamiento de fasteners documentado;
    * las capturas y mediciones finales deben tomarse sobre la build publicada que se use para evaluación;
    * los resultados de usabilidad y rendimiento deben interpretarse según las sesiones documentadas en el informe final y los anexos de validación.
...
### No puedo cerrar un panel
    ...
    * deslizar hacia abajo sobre la ficha inferior si está abierta;
    * usar el botón de cierre de la ficha;
    * presionar `Esc`;
    * usar el botón Atrás del navegador para cerrar la capa activa;
...
## Contacto
**Canal académico:** programa de Ingeniería Multimedia, UNAD.
