...
| -------------------------- | ------------------------------- |
| `<hash>.wasm(.br)`         | ~5–8 MB                         |
| `<hash>.data(.br)`         | ~20–40 MB (depende de texturas) |
| `<hash>.framework.js(.br)` | ~0.3 MB                         |
| **Total descarga**         | **~25–50 MB**                   |

> **Nota:** En builds recientes con `Name Files As Hashes`, Unity ya no exporta `Build.wasm.br` o `Build.data.br` con nombres fijos, sino artefactos hashados resueltos por el `index.html` del build.
