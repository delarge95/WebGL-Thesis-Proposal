---
tipo: indice_estudio
fuente: Informe_final/informe_final.pdf
estado: activo
area: sustentacion
---

# Variante de Estudio Obsidian del Informe Final

Esta variante no reemplaza el informe final. Es el mapa de entrada para estudiar y defender cada seccion con explicaciones ampliadas.

PDF base usado como referencia: [informe_final.pdf](informe_final.pdf)

## Decision de navegacion

La profundizacion se maneja en Obsidian porque permite enlaces entre conceptos, preguntas de defensa, glosarios y fuentes. El PDF formal se conserva como documento de entrega; este archivo funciona como capa clickeable de estudio.

## Mapa por capitulos

| Seccion del informe | Modulo de profundizacion |
| --- | --- |
| Introduccion | [[INF_EST_01_Introduccion]] |
| Planteamiento del problema | [[INF_EST_01_Introduccion#Planteamiento del problema]] |
| Justificacion | [[INF_EST_01_Introduccion#Justificacion]] |
| Objetivos | [[INF_EST_01_Introduccion#Objetivos]] |
| Alcance y limitaciones | [[INF_EST_01_Introduccion#Alcance y limitaciones]] |
| Marco de Referencia | [[INF_EST_02_Marco_Referencia]] |
| Estado del Arte | [[INF_EST_02_Marco_Referencia#Estado del arte]] |
| Marco Teorico | [[INF_EST_02_Marco_Referencia#Marco teorico]] |
| Fundamentos matematicos | [[INF_EST_02_Marco_Referencia#Fundamentos matematicos]] |
| Marco Metodologico | [[INF_EST_03_Marco_Metodologico]] |
| DSR | [[INF_EST_03_Marco_Metodologico#DSR en palabras claras]] |
| Variables | [[INF_EST_03_Marco_Metodologico#Variables]] |
| Diseno comparativo | [[INF_EST_03_Marco_Metodologico#Condicion 2D]] |
| Instrumentos | [[INF_EST_03_Marco_Metodologico#SUS]] |
| Desarrollo e Implementacion | [[INF_EST_04_Desarrollo_Implementacion]] |
| Pipeline CAD a runtime | [[INF_EST_04_Desarrollo_Implementacion#Pipeline CAD a runtime]] |
| MAD-T adaptado | [[INF_EST_04_Desarrollo_Implementacion#MAD-T adaptado]] |
| Pipeline Blender final a Unity | [[INF_EST_30_Pipeline_CAD_MAD_T#Pipeline final Blender a Unity]] |
| UX/UI mobile-first | [[INF_EST_04_Desarrollo_Implementacion#UX/UI mobile-first]] |
| Onboarding procedural | [[INF_EST_31_UX_UI_Mobile_First#Onboarding como reduccion de incertidumbre]] |
| Ficha inferior e info panel | [[INF_EST_31_UX_UI_Mobile_First#Info panel como centro semantico]] |
| Jerarquia de seleccion | [[INF_EST_35_Tooling_Arquitectura_Runtime#Jerarquia de seleccion]] |
| Shaders y entornos | [[INF_EST_04_Desarrollo_Implementacion#Shaders y entornos]] |
| Subsistema termico | [[INF_EST_04_Desarrollo_Implementacion#Subsistema termico]] |
| Resultados y Analisis | [[INF_EST_05_Resultados_Analisis]] |
| Conclusiones | [[INF_EST_06_Conclusiones]] |
| Apendices | [[INF_EST_08_Apendices]] |

## Herramientas transversales

- [[INF_EST_00_Guia_Defensa_y_Lectura]]
- [[INF_EST_20_Catalogo_Secciones_y_Cobertura]]
- [[INF_EST_90_Glosario_Global]]
- [[INF_EST_91_Preguntas_Dificiles_Defensa]]
- [[INF_EST_50_Storytelling_Sustentacion_30min]]
- [[INF_EST_51_Guion_Completo_Sustentacion_30min]]
- [[INF_EST_52_Auditoria_Oratoria_Guion]]
- [[INF_EST_53_Auditoria_Oratoria_v2_Diagnostico]]
- [[INF_EST_54_Guion_Corregido_v2_Parte1]]
- [[INF_EST_55_Guion_Corregido_v2_Parte2]]
- [[MOC_Informe_Final_Estudio_Profundo]]

## Profundizaciones granulares

- [[INF_EST_30_Pipeline_CAD_MAD_T]]
- [[INF_EST_31_UX_UI_Mobile_First]]
- [[INF_EST_32_Iconografia_Procedural_Microinteracciones]]
- [[INF_EST_33_Shaders_ViewModes_Entornos]]
- [[INF_EST_34_Sistema_Termico_Hibrido]]
- [[INF_EST_35_Tooling_Arquitectura_Runtime]]

## Puntos UX agregados en esta revision

- Onboarding procedural: explica gestos, seleccion, ficha, modos y sliders con demos dibujadas por codigo.
- Ficha inferior/info panel: convierte seleccion en lectura tecnica mediante identificacion, especificaciones y ensamblaje.
- Jerarquia de seleccion: diferencia pieza madre, subpieza, grupo de hotspot y fastener para que UI, camara, highlight e isolate no se contradigan.
- Pipeline Blender final a Unity: el runtime debe incluir masters e instancias; el FBX/importador estan preparados, pero el cierre requiere QA en Unity, texturas finales y reporte de importacion.
- Sustentacion y demo: no afirmar metricas, FPS, reducciones, SUS o NASA-TLX hasta congelar la build y medir.
