...
\textit{IL2CPP (\textit{Intermediate Language to C++}).} \textit{Backend} de compilaci\'on de Unity que transpila \textit{bytecode} de C\# a C++, y luego a WASM en el caso de WebGL.
...
\textit{MVP (Producto M\'inimo Viable).} Versi\'on de un producto con las funcionalidades suficientes para validar hip\'otesis esenciales con usuarios reales.
...
\textit{WebAssembly (WASM).} Formato binario ejecutable en navegadores con desempe\~no cercano al nativo, utilizado por Unity para desplegar l\'ogica C\# en WebGL.
