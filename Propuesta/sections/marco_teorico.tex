...
El dise\~no, desarrollo y evaluaci\'on de la plataforma web 3D se fundamenta en un marco te\'orico multidisciplinario que integra conocimientos de ingenier\'ia, dise\~no, psicolog\'ia cognitiva e inform\'atica gr\'afica. El proyecto no se limita a mostrar un modelo tridimensional: busca examinar de forma formativa en qu\'e medida una visualizaci\'on t\'ecnica interactiva puede facilitar la exploraci\'on espacial del hardware, sostener un rendimiento web aceptable y articular una experiencia de usuario m\'as clara que la documentaci\'on 2D est\'atica.

\paragraph{Proposici\'on de trabajo sobre usabilidad y carga de trabajo percibida.} La visualizaci\'on 3D interactiva se plantea como un medio que puede reducir parte de la carga extr\'inseca asociada a la reconstrucci\'on espacial que en soportes 2D debe resolverse mentalmente por el usuario. Esta proposici\'on se explorar\'a mediante tareas representativas, registro de desempe\~no en tareas, KPIs t\'ecnicos, SUS aplicado al prototipo interactivo, NASA-TLX por condici\'on y observaci\'on \textit{Think-Aloud}, sin asumir de antemano una demostraci\'on causal concluyente.
...
\paragraph{Carga germana.} Es el esfuerzo que s\'i contribuye a construir esquemas mentales \'utiles. En una interfaz 3D bien dise\~nada, los recursos cognitivos liberados por la reducci\'on de carga extr\'inseca pueden redirigirse a comprender mejor la funci\'on, ubicaci\'on y relaci\'on entre componentes del ensamblaje.

En el contexto del prototipo, la interfaz 3D orbital y la consulta contextual de piezas buscan precisamente reducir la carga extr\'inseca y favorecer una carga germana orientada a la comprensi\'on espacial y funcional del ensamblaje.
...
\]
donde $R_{\text{texture}}$ es la resoluci\'on efectiva aplicada sobre un tramo de referencia y $L_{\text{mundo}}$ es la longitud real correspondiente en el modelo. Si $\rho$ cambia bruscamente entre piezas, algunas se perciben borrosas y otras resultan innecesariamente costosas en memoria.
...
El renderizado f\'isicamente basado parte de la \textit{rendering equation}, formulada por Kajiya (1986) y adoptada como fundamento general de los modelos BRDF utilizados en renderizado en tiempo real (Akenine-M\"oller et al., 2018):
\[
...
Para reducir costo computacional, el t\'ermino de Fresnel suele aproximarse mediante la expresi\'on de Schlick (1994):
\[
...
La distribuci\'on de microfacetas adoptada com\'unmente en PBR en tiempo real es GGX (Walter et al., 2007; Karis, 2013):
\[
...
\paragraph{Conservaci\'on de energ\'ia.}
...
En la aplicaci\'on final, el modo \texttt{Realistic} no reimplementa manualmente toda la BRDF microfacetada desde cero. El proyecto delega el c\'alculo PBR base al \textit{pipeline} de Unity URP, mientras que los shaders propios a\~naden l\'ogica espec\'ifica del sistema, como planos de corte globales y modos de inspecci\'on (Unity Technologies, 2024b). Por ello, las ecuaciones anteriores se presentan como fundamento matem\'atico del modelo de iluminaci\'on empleado por URP y no como afirmaci\'on de un \textit{solver} BRDF completamente escrito desde cero dentro del proyecto.
