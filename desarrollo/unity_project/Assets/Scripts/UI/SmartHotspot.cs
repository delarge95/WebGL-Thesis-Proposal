...
public class SmartHotspot
{
    ...
    private Vector2 _lastClickPanelPosition;
    ...
    private const float DoubleTapThreshold = 0.42f;
    private const float DoubleTapMaxDistance = 56f;
    ...
    private void OnClick(ClickEvent evt)
    {
        ...
        bool manualDoubleTap = now - _lastClickTime <= DoubleTapThreshold
            && (panelPosition - _lastClickPanelPosition).sqrMagnitude <= DoubleTapMaxDistance * DoubleTapMaxDistance;

        if (evt.clickCount >= 2 || manualDoubleTap)
        {
            _lastClickTime = -999f;
        ...
        _lastClickTime = now;
        _lastClickPanelPosition = panelPosition;
    ...
    private string ResolveHotspotLabel()
    {
        ...
        {
            return AppLanguageManager.TranslateStatic(_labelOverride);
        }
        ...
        if (_targetPart != null && _targetPart.Data != null)
        {
            ...
            {
                return AppLanguageManager.TranslateStatic(_targetPart.Data.hotspotLabel);
            }

            if (!string.IsNullOrWhiteSpace(_targetPart.Data.partName))
            {
                return AppLanguageManager.TranslateStatic(_targetPart.Data.partName);
            }
        }

        return AppLanguageManager.TranslateStatic((_target != null ? _target.name : "Unknown").Replace("_", " "));
    }
